...
The CLI is the interface. The browser extension exists to do the one thing a CLI cannot: read the
ChatGPT and Claude tabs you explicitly track.

```powershell
npm ci
npm run build:individual
shadowqa-individual setup
shadowqa-individual serve      # leave running
shadowqa-individual pair       # connect the side panel
...
| --- | --- | --- |
| Connect | `shadowqa-individual setup` | Detects the Claude Code and Codex CLIs and their local session files, registers the native messaging host for your unpacked extension, and records the automation mode and execution backend. |
| Watch | `shadowqa-individual status` | Shows the tracked conversations, the subscribed coding sessions, the backends and any errors. |
| Read context | `shadowqa-individual context <project>` | Every captured turn, with its source and whether the site gave a real timestamp. |
| Extract | `shadowqa-individual extract <project>` | Gemini turns the turns into requirements, constraints, decisions, suggestions, open questions and conflicts, each citing the turns it came from. |
| Correct | `confirm` / `reject` / `revise` | Your correction is marked and never overwritten by a later extraction. |
| Plan | `shadowqa-individual plan <project> -o "…"` | A plan grounded in the confirmed context **and** the repository at its current commit: scope, affected files, steps, acceptance criteria, tests, unresolved questions. |
| Approve | `shadowqa-individual approve <plan>` | Binds the exact plan digest, base commit, backend and policy version. |
...
| Open it | `shadowqa-individual open <task>` | Prints `claude --resume …`, `codex resume …` or `opencode run -s …` for the same session, to paste into your IDE terminal. |
...
| Deliver | `shadowqa-individual diff <task>` / `pr <task>` | A local `shadowqa/<task>` branch built with Git plumbing; a pull request only if you connected GitHub. |
| Keep watching | `watch`, `scan`, `findings`, `repair` | Saved-file observation, scans, fingerprinted findings and bounded repair. |
...
individual/
  ...
  cli/             shadowqa-individual — the main interface
```

`core`, `service`, `companion` and `cli` are TypeScript compiled by `tsconfig.individual.json` into
`dist-individual/`. They import the business modules directly (`src/db`, `src/memory`,
`src/model/gemini.ts`, `src/runner/process.ts`, `src/runner/workspace.ts`, `src/qa/findings.ts`),
so the two editions share one schema and one set of guarantees.
...
4. Copy the extension ID, then `shadowqa-individual companion install <id>`
5. `shadowqa-individual pair`, and type the code into the panel
...
Both read only the sessions you subscribe to with `shadowqa-individual sessions add`.
...
`shadowqa-individual setup` writes `~/.shadowqa-individual/individual.env` (mode `0600`).
...
| `GITHUB_TOKEN` | Only needed for `shadowqa-individual pr`. |
...
- **No container isolation.** The agent runs in an isolated *workspace*, not a sandbox. It is your
  ...
  this edition.
- **The scheduled scan needs the service running.** Nothing happens while `serve` is stopped; tasks
...
- **An unresolved question does not block approval here.** The team edition refuses a plan that
  ...
  In this edition they are the same person, so `approve` warns and asks rather than refusing.
