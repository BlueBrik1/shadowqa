  ...
  return (
    <Panel eyebrow="Agent" title="Latency, models & audit trail" testid="cc-agent">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div>
          <div className="text-[10px] uppercase tracking-[0.12em] text-[#6f6c66] font-sans mb-2">LLM calls</div>
          <div className="grid gap-1" data-testid="cc-llm-calls">
            {(telemetry?.llm_calls || []).slice(0, 6).map((c, i) => (
              <div key={`${c.ts}${i}`} className="grid grid-cols-[auto_1fr_auto] items-center gap-3 text-[11.5px] font-sans border-t border-[#f4f1ea]/[0.05] pt-1">
                <span className="text-[#a9a59d]">{c.purpose}</span><span className="text-[#6f6c66] truncate" title={c.model}>{c.model}</span><span className={c.ok ? "text-[#3dd68c]" : "text-[#e5484d]"}>{c.ok ? ms(c.latency_ms) : "failed"}</span>
              </div>
            ))}
        ...
        <div>
          <div className="text-[10px] uppercase tracking-[0.12em] text-[#6f6c66] font-sans mb-2">Audit trail</div>
          <div className="grid gap-1 max-h-48 overflow-auto pr-1" data-testid="cc-audit">
            {(audit || []).slice(0, 14).map((e, i) => (
              <div key={`${e.ts}${i}`} className="grid grid-cols-[auto_1fr_auto] items-center gap-3 text-[11.5px] font-sans border-t border-[#f4f1ea]/[0.05] pt-1">
                <span className="text-[#6f6c66]">{clock(e.ts)}</span><span className="text-[#f4f1ea] truncate">{e.action}</span><span className="text-[#6f6c66] truncate max-w-[120px]" title={e.actor}>{e.actor}</span>
              </div>
            ))}
...
export function ContextPanel({ incidents }) {
  ...
  return (
    <Panel eyebrow="Why the browser" title="Context a chatbox never receives" testid="cc-context">
      <p className="text-[11.5px] text-[#a9a59d] mb-3">Every incident is grounded in signals that only exist where the developer is already working — the running app. Seen so far across {(incidents || []).length} incident{(incidents || []).length === 1 ? "" : "s"}:</p>
      <div className="grid sm:grid-cols-2 gap-1.5" data-testid="cc-signal-kinds">
        {kinds.map((k) => (
          <div key={k} className={`flex items-center justify-between gap-3 rounded border px-3 py-1.5 text-[12px] ${counts[k] ? "border-[#f4f1ea]/30 bg-[#f4f1ea]/[0.06]" : "border-[#f4f1ea]/[0.06] opacity-60"}`}>
            <span><span className="text-[#f4f1ea]">{SIGNAL_META[k][0]}</span> <span className="text-[#6f6c66]">· {SIGNAL_META[k][1]}</span></span>
            <span className="font-sans text-[#f4f1ea]">{counts[k] || 0}</span>
          </div>
        ))}
      </div>
    </Panel>
