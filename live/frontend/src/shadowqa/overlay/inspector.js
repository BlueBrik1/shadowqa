...
export function renderInspector(state) {
  ...
  return `<div class="drawer-head"><span class="led"></span><span class="brand">ShadowQA Live · Inspector</span><p class="sub mono">${inc ? `${esc(inc.id)} · ${esc(inc.status)}` : "no active incident"}</p>${state.recent?.length ? `<select class="btn mono" data-action="select-incident" data-testid="sqa-incident-select">${state.recent.map((r) => `<option value="${esc(r.id)}" ${inc && r.id === inc.id ? "selected" : ""}>${esc(r.created_at.slice(11, 19))} · ${esc(r.title)} · ${esc(r.status)}</option>`).join("")}</select>` : ""}<button class="close" data-action="close-inspector" data-testid="sqa-inspector-close">✕</button></div>
