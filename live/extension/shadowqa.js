/* ShadowQA runtime SDK â€” built from frontend/src/shadowqa */
(() => {
...
.card-head .brand::before { content: "â—ˆ "; }
...
.badge-â€” { color: var(--muted); border-color: var(--line); }
...
.drawer-head .brand::before { content: "â—ˆ "; }
...
`;
  ...
  function renderInspector(state) {
    ...
    return `<div class="drawer-head"><span class="led"></span><span class="brand">ShadowQA Live · Inspector</span><p class="sub mono">${inc ? `${escapeHtml(inc.id)} \xB7 ${escapeHtml(inc.status)}` : "no active incident"}</p>${state.recent?.length ? `<select class="btn mono" data-action="select-incident" data-testid="sqa-incident-select">${state.recent.map((r) => `<option value="${escapeHtml(r.id)}" ${inc && r.id === inc.id ? "selected" : ""}>${escapeHtml(r.created_at.slice(11, 19))} \xB7 ${escapeHtml(r.title)} \xB7 ${escapeHtml(r.status)}</option>`).join("")}</select>` : ""}<button class="close" data-action="close-inspector" data-testid="sqa-inspector-close">\u2715</button></div>
  ...
  var head = (led, phase) => `<div class="card-head"><span class="led ${led}"></span><span class="brand">ShadowQA Live</span><span class="phase">${escapeHtml(phase)}</span></div>`;
